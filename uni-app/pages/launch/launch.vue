<template>
  <view class="launch-page">
    ...
    <view class="launch-btn-wrap">
      <button class="launch-btn" @click="handleLaunch">🚀 发射至星际</button>
      <view class="launch-hint">信件将加密存储，在指定日期自动送达</view>
    </view>
...
export default {
  ...
  methods: {
    ...
    proceedWithLaunch(contacts) {
      ...
      const deliveryDate = this.getFutureDate(years);
      const deliveryDateObj = new Date();
      deliveryDateObj.setFullYear(deliveryDateObj.getFullYear() + years);
      const duration = this.getDateDiff(deliveryDateObj);
      ...
      const sentLetter = {
        ...
        deliveryDate,
        deliveryTimestamp: deliveryDateObj.getTime(),
        years,
      };
      ...
      this.successDate = deliveryDate;
      this.successChannel = channelNames[channel];
      this.successDuration = duration;

      if (channel === 'unbreakable') {
...
<style>
/* ===== Launch Page ===== */
.launch-page { padding:0 20px 40px; min-height:100vh; }
...
.keyword-input-wrap { display:flex; gap:8px; align-items:center; }
.keyword-input {
  flex:1; padding:10px 14px; background:var(--glass); border:1px solid var(--glass-bd);
  border-radius:10px; color:var(--text-1); font-size:14px; font-family:inherit; outline:none;
  transition:all .3s;
}
